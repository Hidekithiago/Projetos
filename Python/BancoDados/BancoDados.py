# pip3 install mysql-connector-python install pyodbc
#import mysql.connector
import pyodbc
from datetime import datetime

mysql_homologMysql = { #Configuracao para poder fazer a chamada no connect
    'host': 'localhost',
    'database': '',
    'user': 'root',
    'password': 'root',
...
mysql_homologSqlServer = ("Driver={ODBC Driver 17 for SQL Server};"
            "Server=SMAR01,1433;"
            "Database=MX_Agenciamento;"
            "UID=hideki;"
            "PWD=@genc!am&nto2K22;")
...
class SqlServer():
    ...
    def connect(self, auth):
        ''' Try to connect to a database with auth params defined in config file '''
        try:            
            self.connection = pyodbc.connect(auth)
            cursor = self.connection.cursor()
    ...
    def VerificaUltimaExecucaoRobo(self):
        query = '''Select TOP(1) CAST(GETDATE() - cast(DataHoraCriacao as datetime) as TIME) 
                    from LogRobos 
                    where StatusRoboId = 3 
                    order by LogRoboId desc'''
        cursor = self.connection.cursor()
        cursor.execute(query)
        records = cursor.fetchone()
        return int(records[0].strftime("%H"))
